                    ...
                    <div class="panel-control">
                        ...
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu dropdown-menu-right">
                                <li><a href="javascript:void(0)" class="time-change" data-value="12">12 Hours</a></li>
                                <li><a href="javascript:void(0)" class="time-change" data-value="24">24 Hours</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
                                ...
                                <div class="col-xs-5">
                                    <div class="media">
                                        <div class="media-left">
                                            <span class="text-3x text-thin text-main">20.0</span>
                                        </div>
                                ...
                                @for($i=0,$j=0;$i<3;$i++)
                                    @foreach($medication as $m=>$v)
                                        ...
                                        <div class="col-sm-4 border-right">
                                            <p class="text-lg mar-no activity1">{{$v['value']}}<span class="percentage">%</span></p>
                                            {{$v['name']}}
                                        </div>
                                        <?php $j++; ?>
                                    @endforeach
                    ...
                    <div class="col-sm-8">
                        ...
                        <ul class="list-unstyled text-center bord-top pad-top mar-no row">
                            ...
                            <li class="col-sm-4">
                                ...
                                <!-- <strong>°C</strong>  -->
                                <p class="text-sm text-muted mar-no" style="color: #f1709c">Calories </p>
                            </li>
                        </ul>
                    </div>
                </div>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        chart_1_data=[];
        charts_data.chart1.count=0;
        charts_data.chart1.category=[];
        
        chart_2_data=[];
        charts_data.chart2.count=0;
        charts_data.chart2.category=[];
        ...
        today_mom=start;
        time_flag=-1;
        t1=new Date(start.format('YYYY/MM/DD')+" UTC").getTime();
        t2=new Date(start.format('YYYY/MM/DD')+" UTC").getTime();
...
<script>
    ...
    function getWhereAboutData(){
        whereaboutdata.whd_count=charts_data.chart1.count;
        $.ajax({
            type:"GET",
            dataType: 'json',
    ...
    function getAppliancessData(){
        whereaboutdata.acd_count=charts_data.chart2.count;
        $.ajax({
            type:"GET",
            dataType: 'json',
    ...
    function getNotificationData(){
        ...
        $.ajax({
            ...
            var giCount=1;
            notification_count=notification_count+data.data.count;
            for(var i in data.data.data){
                //console.log(data.data[i].createdOn);
                var d=new Date(parseInt(data.data.data[i].createdOn));
                table.rows.add( [ {
                    "0": giCount,
                    "1": '<span class="sensor_name">'+(room_name[data.data.data[i].sensore_name])+'</span>',
                    "2": '<a class="btn-link" href="#">'+data.data.data[i].action+'</a>',
    ...
    function suceessWhereAboutData(res){
        if(res.count > 0){
            charts_data.chart1.count=charts_data.chart1.count + res.count;
            $.each(res.room, function( index, value ) {
                var key_exist=$.inArray(index, charts_data.chart1.category);
                if( key_exist === -1){
                    charts_data.chart1.category.push(index);
                }
                var temp={"category":index,"segments":[]};
                var row_data=[];
                $.each(res.room_data[index], function( key, value ) {
                    if(value.status==1){
                        if(time_flag==-1 || Math.floor(millisToMinutesAndSeconds(value.createdOn-t1))<=time_flag){
                            var obj={
                ...
                temp.segments=row_data;
                if(key_exist === -1){
                    chart_1_data.push(temp);
                }else if(row_data.length>0){
                    chart_1_data[key_exist].segments=$.merge(chart_1_data[key_exist].segments,row_data);
                }
            });
        }
        chart.startDate=today_mom.format('YYYY-MM-DD')+" 00:00:00";
        chart.dataProvider=chart_1_data;
        chart.validateData();
    }
    function suceessAppalincessData(res){
        if(res.count > 0){
            charts_data.chart2.count=charts_data.chart2.count + res.count;
            $.each(res.room, function( index, value ) {
                var key_exist1=$.inArray(index, charts_data.chart2.category);
                if( key_exist1 === -1){
                    charts_data.chart2.category.push(index);
                }
                var temp={"category":index,"segments":[]};
                var row_data=[];
                ...
                temp.segments=row_data;
                if(key_exist1 === -1){
                    chart_2_data.push(temp);
                }else if(row_data.length>0){
                    chart_2_data[key_exist1].segments=$.merge(chart_2_data[key_exist1].segments,row_data);
                }
            });
        }    
        chart1.startDate=today_mom.format('YYYY-MM-DD')+" 00:00:00";
        chart1.dataProvider=chart_2_data;
        chart1.validateData();
    }
